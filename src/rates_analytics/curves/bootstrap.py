...
def bootstrap_curve(
    ...
    curve = PiecewiseCurve(
        ...
        discount_factors=[_helper_df(PiecewiseCurve([first.maturity_years], [1.0], interpolation), first)],
...
def projection_vs_discount_report(
    ...
    *,
    ...
    float_leg_pv = _floating_leg_pv(projection_curve, discount_curve, maturity=5.0, step=0.25, notional=notional)
    self_discount_pv = _floating_leg_pv(discount_curve, discount_curve, maturity=5.0, step=0.25, notional=notional)
    return {
...
def _floating_leg_pv(
    ...
    *,
    ...
    for accrual, payment in zip(accruals, payments, strict=True):
        pv += notional * accrual * projection_curve.forward_rate(prev, payment) * discount_curve.discount(payment)
        prev = payment
...
def _solve_swap_df(curve: PiecewiseCurve, maturity: float, par_rate: float, fixed_leg_step: float) -> float:
    payment_dates = _payment_schedule(maturity, fixed_leg_step)
    accruals = _accruals(payment_dates)
    known = sum(accrual * curve.discount(payment) for accrual, payment in zip(accruals[:-1], payment_dates[:-1], strict=True))
